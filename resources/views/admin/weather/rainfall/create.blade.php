...
<div class="bg-white shadow rounded-lg p-6">
    <form action="{{ route('admin.weather.rainfall.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select id="weather_station_id" name="weather_station_id" required class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('weather_station_id') border-red-500 @enderror">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="datetime-local" id="recorded_at" name="recorded_at" required value="{{ old('recorded_at') ?? now()->format('Y-m-d\TH:i') }}"
                    class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('recorded_at') border-red-500 @enderror">
                @error('recorded_at')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select id="data_source" name="data_source" required
                    class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('data_source') border-red-500 @enderror">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="number" step="0.01" min="0" id="rainfall_amount" name="rainfall_amount" required
                    ...
                    class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('rainfall_amount') border-red-500 @enderror">
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0" id="intensity" name="intensity"
                    value="{{ old('intensity') }}"
                    class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500 @error('intensity') border-red-500 @enderror">
                @error('intensity')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
